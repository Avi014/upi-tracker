...
{text[:50000]}"""
        ...
        # --- Handle Image (Receipt Scanner) ---
        elif file and file.filename.lower().endswith(('.png', '.jpg', '.jpeg')):
            try:
                if not os.environ.get("GEMINI_API_KEY") or not client:
                    flash("Gemini API Key is missing. Cannot scan receipts. Please set GEMINI_API_KEY.", "danger")
                    return redirect(request.url)
                ...
                response = client.models.generate_content(
                    model='gemini-2.5-flash',
                    contents=[
                        "Analyze this receipt. Return ONLY a valid JSON object with these exact keys: 'merchant' (string), 'date' (YYYY-MM-DD), 'amount' (float). Do not return markdown, backticks, or any other text.",
                        image
                    ]
                )
                ...
                date_val = pd.to_datetime(data['date']).date()
                amount_val = float(data['amount'])
                merchant_val = data['merchant']
                category_val = auto_categorize(merchant_val)
                
                conn = get_db_connection()
                cur = conn.cursor()
                cur.execute('SELECT id FROM transactions WHERE user_id = %s AND date = %s AND merchant = %s AND amount = %s', (current_user.id, date_val, merchant_val, amount_val))
                if not cur.fetchone():
                    cur.execute('''
                        INSERT INTO transactions (user_id, amount, date, merchant, category)
                        VALUES (%s, %s, %s, %s, %s)
                    ''', (current_user.id, amount_val, date_val, merchant_val, category_val))
                    conn.commit()
                    flash(f'Receipt scanned! Added spend at {merchant_val}', 'success')
                else:
                    flash(f'Receipt already uploaded (Duplicate ignored).', 'info')
                cur.close()
                conn.close()
                return redirect(url_for('dashboard'))
            ...
            flash('Invalid file format. Please upload a PDF, CSV, or Image (PNG/JPG).', 'danger')
...
def ai_chat():
    if not os.environ.get("GEMINI_API_KEY"):
        return {"response": "API Key missing. Please set GEMINI_API_KEY to use the AI Advisor."}
        
    user_message = request.json.get('message', '')
    
    conn = get_db_connection()
    cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    cur.execute('SELECT date, merchant, category, amount FROM transactions WHERE user_id = %s ORDER BY date DESC LIMIT 50', (current_user.id,))
    txns = cur.fetchall()
    cur.close()
    conn.close()
    
    context = "User's recent transactions (Amount in INR):\n"
    for t in txns:
        context += f"- {t['date']} | {t['merchant']} | {t['category']} | ₹{t['amount']}\n"
        
    prompt = f"""You are a helpful and friendly AI financial advisor for the 'UPI Leak Detector' app.
The user asked: "{user_message}"

Here is their recent transaction data context:
{context}

Provide a short, helpful, and insightful response (max 3 sentences) based ONLY on the data provided. Use a conversational tone. Do not use markdown."""

    try:
        if not client:
            return {"response": "API client is not initialized."}
        response = client.models.generate_content(
            model='gemini-2.5-flash',
            contents=prompt
        )
